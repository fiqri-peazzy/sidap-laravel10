...
<ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
    ...
    <!-- Nav Item - Event/Turnamen -->
    <li class="nav-item {{ request()->routeIs('event.*') ? 'active' : '' }}">
        <a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#collapseEvent"
            aria-expanded="{{ request()->routeIs('event.*') ? 'true' : 'false' }}" aria-controls="collapseEvent">
            <i class="fas fa-fw fa-calendar-alt"></i>
            <span>Event & Turnamen</span>
        </a>
        <div id="collapseEvent" class="collapse {{ request()->routeIs('event.*') ? 'show' : '' }}"
            aria-labelledby="headingEvent" data-parent="#accordionSidebar">
            <div class="bg-white py-2 collapse-inner rounded">
                <h6 class="collapse-header">Manajemen Event:</h6>
                <a class="collapse-item {{ request()->routeIs('event.index') ? 'active' : '' }}"
                    href="{{ route('event.index') }}">Daftar Event</a>
                <a class="collapse-item {{ request()->routeIs('event.create') ? 'active' : '' }}"
                    href="{{ route('event.create') }}">Buat Event</a>
                <a class="collapse-item {{ request()->routeIs('event.pendaftaran') ? 'active' : '' }}"
